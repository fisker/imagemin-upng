...
test('optimize PNG', async t => {
  const buf = fs.readFileSync(path.join(__dirname, 'fixtures', 'png.png'))
  const data = await m()(buf)

  fs.writeFileSync(path.join(__dirname, 'fixtures', 'png-compressed.png'), data)
...
test('optimize APNG', async t => {
  const buf = fs.readFileSync(path.join(__dirname, 'fixtures', 'apng.png'))
  const data = await m()(buf)
